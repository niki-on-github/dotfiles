...
case "$1" in
	*youtube.com/watch*|*youtube.com/playlist*|*youtu.be*|*hooktube.com*|*bitchute.com*|*webm)
		...
		setsid mpv --x11-name="floating" --title="floating" --pause --ytdl-format="bestvideo[height<=?480]+bestaudio/best" --speed=1.8 --ontop --no-border --force-window --autofit=640x480 --geometry=-15-15 "$1" >/dev/null 2>&1 & ;;
    *mkv|*mp4)
        notify-send "Play Video" "$1"
